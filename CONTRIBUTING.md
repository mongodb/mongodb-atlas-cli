...
- Run `E2E_TAGS=cloudmanager,remote,generic make e2e-test` will run end to end tests against an Cloud Manager instance.<br />
  Please remember to: (a) have a running automation agent, and (b) to set MCLI_* variables to point to your Cloud Manager instance.
...
- Add `Use` - Shows the command and arguments if applicable. Will show up in 'help' output.
- Add `Short` - Briefly describes the command. Will show up in 'help' output.
- Add `Example` - Example of how to use the command. Will show up in 'help' output.
