Write-Output 'Env...'
reg add "HKCU\Environment" /v GOROOT /t REG_EXPAND_SZ /d $(go env GOROOT) /f
reg add "HKCU\Environment" /v GOBIN /t REG_EXPAND_SZ /d "%GOROOT%\bin" /f
reg add "HKCU\Environment" /v GOPATH /t REG_EXPAND_SZ /d "%USERPROFILE%\go" /f
reg add "HKCU\Environment" /v CGO_ENABLED /t REG_EXPAND_SZ /d "0" /f

$reg_value = reg query "HKCU\Environment" /v PATH
