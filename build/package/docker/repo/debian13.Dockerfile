...
RUN set -eux; \
	...
	echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-${pgp_server_version}.gpg ] ${mongo_repo}/apt/debian trixie/${mongo_package}/${server_version} main" | tee /etc/apt/sources.list.d/${mongo_package}-${server_version}.list; \
	apt-get update; \
