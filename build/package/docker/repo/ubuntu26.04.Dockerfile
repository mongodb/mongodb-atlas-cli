...
RUN set -eux; \
	...
	echo "deb [ arch=amd64,arm64 ] ${mongo_repo}/apt/ubuntu resolute/${mongo_package}/${server_version} multiverse" | tee /etc/apt/sources.list.d/${mongo_package}-${server_version}.list; \
	apt-get update; \
