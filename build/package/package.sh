...
if [[ "${unstable-}" == "-unstable" ]]; then
  ...
  ./bin/goreleaser --config "${goreleaser_config:?}" --rm-dist --snapshot -p 1 --debug
else
  # avoid race conditions on the notarization step by using `-p 1`
  ./bin/goreleaser --config "${goreleaser_config:?}" --rm-dist --release-notes "${changelog_file:?}" -p 1 --debug
fi
