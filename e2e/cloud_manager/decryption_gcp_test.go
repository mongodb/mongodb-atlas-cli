...
func TestDecryptWithGCP(t *testing.T) {
	...
	err = dumpToTemp(filesGCP, generateFileName(gcpTestsInputDir, "input"), inputFile)
	req.NoError(err)

	expectedContents, err := filesGCP.ReadFile(generateFileName(gcpTestsInputDir, "output"))
	req.NoError(err)
