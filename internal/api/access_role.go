...
//go:generate mockgen -destination=../mocks/api/mock_access_role.go -package=mocks github.com/mongodb/mongodb-atlas-cli/internal/api CloudProviderAccessRoleCreator,CloudProviderAccessRoleAuthorizer,CloudProviderAccessRoleLister,CloudProviderAccessRoleDeauthorizer
...
func (s *Store) CreateCloudProviderAccessRole(groupID, provider string) (*atlasv2.CloudProviderAccessRole, error) {
	...
	result, _, err := s.clientv2.CloudProviderAccessApi.CreateCloudProviderAccessRole(s.ctx, groupID).CloudProviderAccessRole(req).Execute()
	return result, err
...
func (s *Store) AuthorizeCloudProviderAccessRole(groupID, roleID string, req *atlas.CloudProviderAuthorizationRequest) (*atlasv2.CloudProviderAccessRole, error) {
	...
	result, _, err := s.clientv2.CloudProviderAccessApi.AuthorizeCloudProviderAccessRole(s.ctx, groupID, roleID).CloudProviderAccessRole(role).Execute()
	return result, err
}
