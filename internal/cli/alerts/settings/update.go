...
//go:generate go tool go.uber.org/mock/mockgen -typed -destination=update_mock_test.go -package=settings -source=update.go
...
type UpdateOpts struct {
	...
	cli.OutputOpts
	ConfigOpts
	store    AlertConfigurationUpdater
	alertID  string
	filename string
	fs       afero.Fs
}
  ...
  atlas alerts settings update 5d1113b25a115342acc2d1aa --file alerts.json`,
		PreRunE: func(cmd *cobra.Command, _ []string) error {
			return opts.PreRunE(
				...
				func() error {
					if opts.filename == "" {
						return opts.validateConfigOpts()
					}
					return nil
				},
