...
func (opts *Opts) askRequiredFlags() {
	if opts.dbUsername == "" {
		dbUsername := ""
		...
		survey.AskOne(dbUsernamePrompt, &dbUsername)
		opts.dbUsername = "quickStart_" + newAutogeneratedPassword(4)
		if dbUsername != "" {
	...
	if opts.dbUserPassword == "" {
		...
		survey.AskOne(dbPasswordPrompt, &dbPassword)
		opts.dbUserPassword = newAutogeneratedPassword(12)
		if dbPassword != "" {
	...
	if opts.provider == "" {
		...
		survey.AskOne(providerPrompt, &opts.provider)
	}
	...
	if opts.region == "" {
		...
		survey.AskOne(regionPrompt, &opts.region)
	}
}

func newAutogeneratedPassword(length int) string {
