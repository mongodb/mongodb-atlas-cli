...
func (opts *atlasClustersCreateOpts) newProviderSettings() *atlas.ProviderSettings {
	...
	return &atlas.ProviderSettings{
		InstanceSizeName:    opts.clusterTier,
...
func (opts *atlasClustersCreateOpts) providerName() string {
	if opts.clusterTier == atlasM2 || opts.clusterTier == atlasM5 {
		return tenant
	}
...
// create <name> --projectId projectId --provider AWS|GCP|AZURE --region regionName [--members N] [--clusterTier M#] [--diskSizeGB N] [--backup] [--mdbVersion]
func AtlasClustersCreateBuilder() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mongocli atlas cluster create myCluster --projectId=<projectId> --provider AWS --region US_EAST_1 --members 3 --clusterTier M10 --mdbVersion 4.2 --diskSizeGB 10`,
	...
	cmd.Flags().StringVar(&opts.clusterTier, flags.ClusterTier, atlasM2, usage.ClusterTier)
