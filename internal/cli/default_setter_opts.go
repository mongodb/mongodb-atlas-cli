...
type ProjectOrgsLister interface {
	...
	Organization(id string) (*opsmngr.Organization, error)
...
func (opts *DefaultSetterOpts) projects() (ids, names []string, err error) {
	...
	ids, names = omProjects(projects.Results)
	return ids, names, nil
}

// Orgs fetches organizations, filtering by name.
func (opts *DefaultSetterOpts) orgs(filter string) (organizations []*opsmngr.Organization, err error) {
	spin := newSpinner()
	spin.Start()
	defer spin.Stop()
...
func (opts *DefaultSetterOpts) askOrgWithFilter(filter string) error {
	...
	return opts.selectOnPremOrg(orgs)
}
