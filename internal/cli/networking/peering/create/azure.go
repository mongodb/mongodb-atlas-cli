...
//go:generate go tool go.uber.org/mock/mockgen -typed -destination=azure_mock_test.go -package=create -source=azure.go
...
func AzureBuilder() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Before you create an Azure network peering connection, complete the prerequisites listed here: https://www.mongodb.com/docs/atlas/security-vpc-peering/#aws-vpc-peering-prerequisites.
