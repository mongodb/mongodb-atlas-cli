...
//go:generate go tool go.uber.org/mock/mockgen -typed -destination=describe_mock_test.go -package=interfaces -source=describe.go
...
// atlas privateEndpoint(s) azure interface(s) describe <endpointId> [--endpointServiceID endpointServiceID][--projectId projectId].
