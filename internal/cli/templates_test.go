...
func TestTemplates(t *testing.T) {
	missingTemplateTests, err := inner()
	if err != nil {
		log.Fatalf("Error: %v", err)
	}

	if len(missingTemplateTests) > 0 {
		errorString := "Some templates are not being tested:\n"
		for _, templateBuilderFunc := range missingTemplateTests {
			errorString += fmt.Sprintf("- %v\t\t%v\n", templateBuilderFunc.templateIdent, templateBuilderFunc.pkg.Fset.Position(templateBuilderFunc.templateIdent.NamePos))
		}

		assert.Fail(t, errorString)
	}
}
...
func inner() ([]*BuilderFunc, error) {
	missingTemplates := make([]*BuilderFunc, 0)
	...
	for _, builderFunc := range builderFuncs {
		...
		if !templateGotTested {
			missingTemplates = append(missingTemplates, builderFunc)
		}
	}
