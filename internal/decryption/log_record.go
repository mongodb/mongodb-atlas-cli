...
func processLogRecord(decryptConfig *DecryptSection, logLine *AuditLogLine, lineNb int) (bsonData interface{}, keyInvocationCount uint64, err error) {
	...
	if decodeErr != nil {
		return nil, 0, fmt.Errorf("at line %v: %w: %v", lineNb, ErrLogCorrupted, decodeErr.Error())
	}
	...
	if decryptErr != nil {
		return nil, 0, fmt.Errorf("%w at line %v: %v", ErrDecryptionFailure, lineNb, decryptErr.Error())
	}

	decompressedLogRecord, decompressErr := decompress(decryptConfig.compressionMode, decryptedLog)
	if decompressErr != nil {
		return nil, 0, fmt.Errorf("%w at line %v: %v", ErrDecompressionFailure, lineNb, decompressErr.Error())
	}
	...
	if bsonErr := bson.Unmarshal(decompressedLogRecord, &bsonParsedLogRecord); bsonErr != nil {
		return nil, 0, fmt.Errorf("%w at line %v: %v", ErrParse, lineNb, bsonErr.Error())
	}
