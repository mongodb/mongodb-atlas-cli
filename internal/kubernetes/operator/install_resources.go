...
func (ir *InstallResources) InstallConfiguration(ctx context.Context, installConfig *InstallConfig) error {
	...
	for _, config := range configData {
		...
		case "Deployment":
			err = ir.addDeployment(ctx, config, installConfig.Namespace, installConfig.Watch, installConfig.ResourceDeletionProtectionEnabled, installConfig.SubResourceDeletionProtectionEnabled)
			if err != nil {
				return err
			}
		default:
...
func (ir *InstallResources) addDeployment(ctx context.Context, config map[string]interface{}, namespace string, watch []string, deletionProtection, subDeletionProtection bool) error {
