...
func (p *printer) PrintNewVersionAvailable(latestVersion, homebrewCommand string) error {
	var upgradeInstructions string
	if homebrewCommand != "" {
		upgradeInstructions = fmt.Sprintf(`To upgrade, run "brew update && brew upgrade %s".`, homebrewCommand)
	} else {
