...
func TestApprovalHappyPath(t *testing.T) {
	...
	token, err := pledge.WriteApprovalRequest(testSID, "deleteCluster", api.PermissionWrite)
	if err != nil {
		t.Fatal(err)
	}

	// Approve with no pledge (approver has full access).
	if err := pledge.Approve(token, nil, testSID+1); err != nil {
	...
	defer cancel()
	grant, err := pledge.WaitForGrant(ctx, testSID, token)
	if err != nil {
	...
	// Consume.
	if err := pledge.ConsumeGrant(testSID, token, "deleteCluster"); err != nil {
		t.Fatalf("ConsumeGrant: %v", err)
	}

	// Double-consume must fail.
	if err := pledge.ConsumeGrant(testSID, token, "deleteCluster"); !errors.Is(err, pledge.ErrApprovalConsumed) {
...
func TestApprovalNotFound(t *testing.T) {
	withTempStateDir(t)
	if err := pledge.ConsumeGrant(testSID, "deadbeef", "someOp"); !errors.Is(err, pledge.ErrApprovalNotFound) {
...
func TestApprovalApproverUnderPledged(t *testing.T) {
	...
	token, err := pledge.WriteApprovalRequest(testSID, "deleteOrg", api.PermissionAdmin)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := pledge.Approve(token, roPledge, testSID+1); !errors.Is(err, pledge.ErrApproverUnderPledged) {
...
func TestApprovalForgedGrant(t *testing.T) {
	...
	token, err := pledge.WriteApprovalRequest(testSID, "listClusters", api.PermissionRead)
	if err != nil {
		t.Fatal(err)
	}
	if err := pledge.Approve(token, nil, testSID+1); err != nil {
	...
	if err := pledge.ConsumeGrant(testSID, token, "deleteCluster"); !errors.Is(err, pledge.ErrApprovalForged) {
