...
func (s *Store) GetAutomationStatus(projectID string) (*opsmngr.AutomationStatus, error) {
	...
	case config.CloudManagerService, config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).Automation.GetStatus(s.Context(), projectID)
		return result, err
	default:
...
func (s *Store) GetAutomationConfig(projectID string) (*opsmngr.AutomationConfig, error) {
	switch s.service {
	case config.CloudManagerService, config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).Automation.GetConfig(s.Context(), projectID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) UpdateAutomationConfig(projectID string, automationConfig *opsmngr.AutomationConfig) error {
	switch s.service {
	case config.CloudManagerService, config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).Automation.UpdateConfig(s.Context(), projectID, automationConfig)
		return err
	default:
