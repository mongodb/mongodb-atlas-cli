...
type DatabaseUserUpdater interface {
	UpdateDatabaseUser(*atlasv2.DatabaseUser) (*atlasv2.DatabaseUser, error)
}
...
func (s *Store) UpdateDatabaseUser(user *atlasv2.DatabaseUser) (*atlasv2.DatabaseUser, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.clientv2.DatabaseUsersApi.UpdateDatabaseUser(s.ctx, user.GroupId, user.DatabaseName, user.Username).DatabaseUser(*user).Execute()
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) DatabaseUser(authDB, groupID, username string) (*atlasv2.DatabaseUser, error) {
	...
	case config.CloudService, config.CloudGovService:
		...
		result, _, err := s.clientv2.DatabaseUsersApi.GetDatabaseUser(s.ctx, groupID, authDB, escapedUsername).Execute()
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
