...
func (s *Store) StartMonitoring(groupID string, host *opsmngr.Host) (*opsmngr.Host, error) {
	...
	case config.OpsManagerService, config.CloudManagerService:
		result, _, err := s.client.(*opsmngr.Client).Deployments.StartMonitoring(s.Context(), groupID, host)
		return result, err
	default:
...
func (s *Store) StopMonitoring(groupID, hostID string) error {
	switch s.service {
	case config.OpsManagerService, config.CloudManagerService:
		_, err := s.client.(*opsmngr.Client).Deployments.StopMonitoring(s.Context(), groupID, hostID)
		return err
	default:
