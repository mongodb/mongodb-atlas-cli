...
func (s *Store) ListOplogs(options *atlas.ListOptions) (*opsmngr.BackupStores, error) {
	...
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).OplogStoreConfig.List(s.Context(), options)
		return result, err
	default:
...
func (s *Store) GetOplog(oplogID string) (*opsmngr.BackupStore, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).OplogStoreConfig.Get(s.Context(), oplogID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) CreateOplog(oplog *opsmngr.BackupStore) (*opsmngr.BackupStore, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).OplogStoreConfig.Create(s.Context(), oplog)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) UpdateOplog(oplogID string, oplog *opsmngr.BackupStore) (*opsmngr.BackupStore, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).OplogStoreConfig.Update(s.Context(), oplogID, oplog)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) DeleteOplog(oplogID string) error {
	switch s.service {
	case config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).OplogStoreConfig.Delete(s.Context(), oplogID)
		return err
	default:
