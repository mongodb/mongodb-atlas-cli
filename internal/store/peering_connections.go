...
type PeeringConnectionCreator interface {
	...
	CreatePeeringConnection(string, *atlasv2.NetworkContainerPeer) (interface{}, error)
}
...
func (s *Store) CreatePeeringConnection(projectID string, peer *atlasv2.NetworkContainerPeer) (interface{}, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
