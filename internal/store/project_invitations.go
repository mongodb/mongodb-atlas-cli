...
func (s *Store) ProjectInvitations(groupID string, opts *atlas.InvitationOptions) ([]*atlas.Invitation, error) {
	...
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).Projects.Invitations(s.Context(), groupID, opts)
		return result, err
	case config.CloudManagerService, config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).Projects.Invitations(s.Context(), groupID, opts)
		return result, err
	default:
...
func (s *Store) ProjectInvitation(groupID, invitationID string) (*atlas.Invitation, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).Projects.Invitation(s.Context(), groupID, invitationID)
		return result, err
	case config.CloudManagerService, config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).Projects.Invitation(s.Context(), groupID, invitationID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) DeleteProjectInvitation(groupID, invitationID string) error {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		_, err := s.client.(*atlas.Client).Projects.DeleteInvitation(s.Context(), groupID, invitationID)
		return err
	case config.CloudManagerService, config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).Projects.DeleteInvitation(s.Context(), groupID, invitationID)
		return err
	default:
...
func (s *Store) InviteUserToProject(groupID string, invitation *atlas.Invitation) (*atlas.Invitation, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).Projects.InviteUser(s.Context(), groupID, invitation)
		return result, err
	case config.CloudManagerService, config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).Projects.InviteUser(s.Context(), groupID, invitation)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) UpdateProjectInvitation(groupID, invitationID string, invitation *atlas.Invitation) (*atlas.Invitation, error) {
	...
	case config.CloudService, config.CloudGovService:
		if invitationID != "" {
			result, _, err := s.client.(*atlas.Client).Projects.UpdateInvitationByID(s.Context(), groupID, invitationID, invitation)
			return result, err
		}
		result, _, err := s.client.(*atlas.Client).Projects.UpdateInvitation(s.Context(), groupID, invitation)
		return result, err

	case config.CloudManagerService, config.OpsManagerService:
		if invitationID != "" {
			result, _, err := s.client.(*opsmngr.Client).Projects.UpdateInvitationByID(s.Context(), groupID, invitationID, invitation)
			return result, err
		}
		result, _, err := s.client.(*opsmngr.Client).Projects.UpdateInvitation(s.Context(), groupID, invitation)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
