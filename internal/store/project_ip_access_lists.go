...
func (s *Store) CreateProjectIPAccessList(entries []*atlas.ProjectIPAccessList) (*atlas.ProjectIPAccessLists, error) {
	...
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).ProjectIPAccessList.Create(s.Context(), entries[0].GroupID, entries)
		return result, err
	default:
...
func (s *Store) DeleteProjectIPAccessList(projectID, entry string) error {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		_, err := s.client.(*atlas.Client).ProjectIPAccessList.Delete(s.Context(), projectID, entry)
		return err
	default:
...
func (s *Store) ProjectIPAccessLists(projectID string, opts *atlas.ListOptions) (*atlas.ProjectIPAccessLists, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).ProjectIPAccessList.List(s.Context(), projectID, opts)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) IPAccessList(projectID, name string) (*atlas.ProjectIPAccessList, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).ProjectIPAccessList.Get(s.Context(), projectID, name)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
