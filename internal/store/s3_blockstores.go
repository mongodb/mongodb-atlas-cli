...
//go:generate mockgen -destination=../mocks/mock_backup_s3_blockstores.go -package=mocks github.com/mongodb/mongodb-atlas-cli/mongocli/v2/internal/store S3BlockstoresLister,S3BlockstoresDeleter,S3BlockstoresCreator,S3BlockstoresUpdater,S3BlockstoresDescriber
...
func (s *Store) ListS3Blockstores(options *opsmngr.ListOptions) (*opsmngr.S3Blockstores, error) {
	...
	case config.OpsManagerService:
		result, _, err := s.client.S3BlockstoreConfig.List(s.ctx, options)
		return result, err
	default:
...
func (s *Store) DeleteS3Blockstore(blockstoreID string) error {
	switch s.service {
	case config.OpsManagerService:
		_, err := s.client.S3BlockstoreConfig.Delete(s.ctx, blockstoreID)
		return err
	default:
...
func (s *Store) CreateS3Blockstores(blockstore *opsmngr.S3Blockstore) (*opsmngr.S3Blockstore, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.S3BlockstoreConfig.Create(s.ctx, blockstore)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) UpdateS3Blockstores(blockstoreID string, blockstore *opsmngr.S3Blockstore) (*opsmngr.S3Blockstore, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.S3BlockstoreConfig.Update(s.ctx, blockstoreID, blockstore)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) GetS3Blockstore(blockstoreID string) (*opsmngr.S3Blockstore, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.S3BlockstoreConfig.Get(s.ctx, blockstoreID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
