...
func (s *Store) ProjectServerType(projectID string) (*opsmngr.ServerType, error) {
	...
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).ServerUsage.GetServerTypeProject(s.Context(), projectID)
		return result, err
	default:
...
func (s *Store) UpdateProjectServerType(projectID string, serverType *opsmngr.ServerTypeRequest) error {
	switch s.service {
	case config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).ServerUsage.UpdateProjectServerType(s.Context(), projectID, serverType)
		return err
	default:
...
func (s *Store) OrganizationServerType(orgID string) (*opsmngr.ServerType, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).ServerUsage.GetServerTypeOrganization(s.Context(), orgID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) UpdateOrganizationServerType(orgID string, serverType *opsmngr.ServerTypeRequest) error {
	switch s.service {
	case config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).ServerUsage.UpdateOrganizationServerType(s.Context(), orgID, serverType)
		return err
	default:
		return fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) ProjectHostAssignments(projectID string, opts *opsmngr.ServerTypeOptions) (*opsmngr.HostAssignments, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).ServerUsage.ProjectHostAssignments(s.Context(), projectID, opts)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) OrganizationHostAssignments(orgID string, opts *opsmngr.ServerTypeOptions) (*opsmngr.HostAssignments, error) {
	switch s.service {
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).ServerUsage.OrganizationHostAssignments(s.Context(), orgID, opts)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) GenerateSnapshot() error {
	switch s.service {
	case config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).ServerUsage.GenerateDailyUsageSnapshot(s.Context())
		return err
	default:
		return fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) DownloadServerUsageReport(opts *opsmngr.ServerTypeOptions, out io.Writer) error {
	switch s.service {
	case config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).ServerUsageReport.Download(s.Context(), opts, out)
		return err
	default:
		return fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
