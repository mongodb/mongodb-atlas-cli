...
func (s *Store) ServerlessInstances(projectID string, listOps *atlas.ListOptions) (*atlas.ClustersResponse, error) {
	...
	case config.CloudService:
		result, _, err := s.client.(*atlas.Client).ServerlessInstances.List(s.Context(), projectID, listOps)
		return result, err
	default:
...
func (s *Store) ServerlessInstance(projectID, clusterName string) (*atlas.Cluster, error) {
	switch s.service {
	case config.CloudService:
		result, _, err := s.client.(*atlas.Client).ServerlessInstances.Get(s.Context(), projectID, clusterName)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) DeleteServerlessInstance(projectID, name string) error {
	switch s.service {
	case config.CloudService:
		_, err := s.client.(*atlas.Client).ServerlessInstances.Delete(s.Context(), projectID, name)
		return err
	default:
...
func (s *Store) CreateServerlessInstance(projectID string, cluster *atlas.ServerlessCreateRequestParams) (*atlas.Cluster, error) {
	switch s.service {
	case config.CloudService:
		result, _, err := s.client.(*atlas.Client).ServerlessInstances.Create(s.Context(), projectID, cluster)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
