...
//go:generate go tool go.uber.org/mock/mockgen -destination=../mocks/mock_store.go -package=mocks -source=store.go
...
func (s *Store) setAtlasClient() error {
	opts := []atlas.ClientOpt{atlas.SetUserAgent(useragent.UserAgent(version.Version))}
	if s.baseURL != "" {
...
func (s *Store) createV2Client(client *http.Client) error {
	opts := []atlasv2.ClientModifier{
		...
		atlasv2.UseUserAgent(useragent.UserAgent(version.Version)),
...
func (s *Store) createClustersClient(client *http.Client) error {
	opts := []atlasClustersPinned.ClientModifier{
		...
		atlasClustersPinned.UseUserAgent(useragent.UserAgent(version.Version)),
