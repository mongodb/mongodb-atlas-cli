...
import (
	...
	apiv2 "go.mongodb.org/atlas/api/v1alpha"
...
func (s *Store) createV2Client(client *http.Client) error {
	modifiers := []apiv2.ClientModifier{
		apiv2.UseHTTPClient(client),
		apiv2.UseUserAgent(config.UserAgent),
		apiv2.UseDebug(log.IsDebugLevel())}

	if s.baseURL != "" {
		modifiers = append(modifiers, apiv2.UseBaseURL(s.baseURL))
	}
	newClient, err := apiv2.NewClient(modifiers...)
	if err != nil {
		return err
	}
