...
func (s *Store) CreateUser(user *UserRequest) (interface{}, error) {
	...
	case config.CloudService, config.CloudGovService:
		...
		result, _, err := s.client.(*atlas.Client).AtlasUsers.Create(s.Context(), atlasUser)
		return result, err
	case config.OpsManagerService, config.CloudManagerService:
		result, _, err := s.client.(*opsmngr.Client).Users.Create(s.Context(), user.User)
		return result, err
	default:
...
func (s *Store) UserByID(userID string) (interface{}, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).AtlasUsers.Get(s.Context(), userID)
		return result, err
	case config.OpsManagerService, config.CloudManagerService:
		result, _, err := s.client.(*opsmngr.Client).Users.Get(s.Context(), userID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) UserByName(username string) (interface{}, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).AtlasUsers.GetByName(s.Context(), username)
		return result, err
	case config.OpsManagerService, config.CloudManagerService:
		result, _, err := s.client.(*opsmngr.Client).Users.GetByName(s.Context(), username)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) DeleteUser(userID string) error {
	...
	case config.OpsManagerService:
		_, err := s.client.(*opsmngr.Client).Users.Delete(s.Context(), userID)
		return err
	default:
...
func (s *Store) OrganizationUsers(organizationID string, opts *atlas.ListOptions) (interface{}, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).Organizations.Users(s.Context(), organizationID, opts)
		return result, err
	case config.OpsManagerService, config.CloudManagerService:
		result, _, err := s.client.(*opsmngr.Client).Organizations.ListUsers(s.Context(), organizationID, opts)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
...
func (s *Store) TeamUsers(orgID, teamID string) (interface{}, error) {
	switch s.service {
	case config.CloudService, config.CloudGovService:
		result, _, err := s.client.(*atlas.Client).Teams.GetTeamUsersAssigned(s.Context(), orgID, teamID)
		return result, err
	case config.OpsManagerService, config.CloudManagerService:
		result, _, err := s.client.(*opsmngr.Client).Teams.GetTeamUsersAssigned(s.Context(), orgID, teamID)
		return result, err
	default:
		return nil, fmt.Errorf("%w: %s", errUnsupportedService, s.service)
	}
