...
dbName and collection are required only for built-in roles.`
	...
	OrgRole                                   = "User's roles  for the associated organization. To learn which values the Atlas CLI accepts, see the Items Enum for roles in the API spec: https://www.mongodb.com/docs/atlas/reference/api-resources-spec/#operation/createOrganizationInvitation/."
	ProjectRole                               = "User's roles  for the associated project. To learn which values the Atlas CLI accepts, see the Items Enum for roles in the API spec: https://www.mongodb.com/docs/atlas/reference/api-resources-spec/#operation/createProjectInvitation/."
	...
	MatcherOperator                           = "Comparison operator to apply when checking the current metric against matcherValue. Valid values are EQUALS, CONTAINS, STARTS_WITH, ENDS_WITH, NOT_EQUALS, NOT_CONTAINS, REGEX."
	MatcherValue                              = "Value to test with the specified operator. If matcherFieldName is set to TYPE_NAME, you can match on the following values: PRIMARY, SECONDARY, STANDALONE, CONFIG, MONGOS."
	...
	MetricUnits                               = "Units for the threshold value. Value values are BITS, BYTES, DAYS, GIGABITS, GIGABYTES, HOURS, KILOBITS, KILOBYTES, MEGABITS, MEGABYTES, MILLISECONDS, MINUTES, PETABYTES, RAW, SECONDS, TERABYTES."
	...
	NotificationEmailEnabled                  = "Flag indicating whether email notifications should be sent. Configurable for GROUP and USER notification types."
	...
	ServerUsageSkipRedacted                   = "Flag that indicates whether Ops Manager skips redaction of sensitive values. If set to true, Ops Manager logs the original values for hostnames, IP addresses, and namespaces in API responses for this job."
	Sparse                                    = "Flag that indicates whether Atlas should create a sparse index. To learn more about sparse indexes, see https://www.mongodb.com/docs/manual/core/index-sparse/"
	...
	StartASAP                                 = "Flag that, if specified, indicates to start maintenance immediately upon receiving this request. This flag resets to false after Atlas completes maintenance."
	...
	CurrentIP                                 = "Flag that indicates whether to add the IP address from the host that is currently executing the command to the access list. Only applicable for type ipAddress entries."
	CurrentIPSimplified                       = "Flag that indicates whether to add the IP address from the host that is currently executing the command to the access list."
