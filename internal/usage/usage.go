...
dbName and collection are required only for built-in roles.`
	...
	APIKeyRoles                               = "Role or roles that you want to assign to the API key. To assign more than one, you can specify each role with a separate role flag or specify the roles in a comma-separated list with one role flag. Valid values are ORG_OWNER, ORG_MEMBER, ORG_GROUP_CREATOR, ORG_BILLING_ADMIN, and ORG_READ_ONLY." //nolint:gosec // This is just a message not a credential
	...
	APIAccessListIPEntry                      = "IP address that you want to add to the access list for your API key. To add more than one IP address, you can specify each address with a separate ip flag or specify the addresses in a comma-separated list with one ip flag. You can't set both cidr and ip in the same command."
	NetworkAccessListIPEntry                  = "IP address to be allowed to access the deployment."
	AccessListCIDREntry                       = "Access list entry in CIDR notation to be added for your API key. To add more than one entry, you can specify each entry with a separate cidr flag or specify the entries in a comma-separated list with one cidr flag. You can't set both cidr and ip in the same command."
