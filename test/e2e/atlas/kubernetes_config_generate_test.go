...
func TestProjectWithNonDefaultAlertConf(t *testing.T) {
	...
	t.Run("Change project alert config and generate", func(t *testing.T) {
		cmd := exec.Command(cliPath,
			...
			fmt.Sprintf("--notificationSmsEnabled=%t", pointer.GetOrZero(newAlertConfig.Notifications[0].SMSEnabled)),
			fmt.Sprintf("--notificationEmailEnabled=%t", pointer.GetOrZero(newAlertConfig.Notifications[0].EmailEnabled)),
			"--matcherFieldNames",
