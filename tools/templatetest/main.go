...
{{.NestingTest.A1.B1.C1}} {{.NestingTest.A1.B2.C2}} {{.NestingTest.A1.B2.C3}}
{{- range .RangeNest1}}
{{- range .RangeNest2}}
{{.LGTM}}
{{- end}}
...
func (c *TemplateCallTree) String() string {
	...
	if c.listType != nil {
		out += c.listType.structType.Fprint(0)
	}

	if c.structType != nil {
		out += c.structType.Fprint(0)
	}
...
func (c *TemplateCallTreeEntry) Fprint(depth int) string {
	...
	for key, value := range c.fields {
		if value.listType != nil {
			...
			out += value.listType.structType.Fprint(depth + 1)
		}
...
func inner() error {
	parseTree, err := parse.Parse("templ", describeTemplate, "{{", "}}", templateFuncs)
	if err != nil {
		return err
	}
...
func buildTree(root *TemplateCallTree, node parse.Node) error {
	...
	case *parse.IdentifierNode:
		...
		// we might want to care about them in a later stadium if we want to validate function calls too
		return nil
	default:
